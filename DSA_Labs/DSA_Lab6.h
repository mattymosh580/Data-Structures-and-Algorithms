...
public:
	...
	void PopulateLetterValues(const int* _letterValues) {
		...
		for (int i = 0; i < 26; i++)
		{
			this->mLetterValues[i] = _letterValues[i];
		}
	...
	int GetLetterValue(char _letter) const {
		...
		for (int i = 0; i < 26; i++)
		{
			if (key == (int)_letter) return mLetterValues[i];
	...
	std::pair<std::string, int> CreatePair(const std::string& _word) const {
		...
		return pair<string, int>(_word, GetWordValue(_word));
	...
	void LoadWords(const char* _filename) {
		...
		while (!file.eof())
		{
			...
			mScrabbleMap.insert(pair<string, int>(temp, GetWordValue(temp)));
		}
	...
	int FindValueInMap(const std::string& _word) {
		...
		if (mScrabbleMap[_word]) return mScrabbleMap.find(_word)->second;
